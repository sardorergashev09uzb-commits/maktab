...
import { Coins, Plus, TrendingUp, Award, Clock, ArrowUpRight, ArrowDownRight, Gift, X } from 'lucide-react';

export default function CoinsPage() {
  const { user } = useAuth();
  const roles = user?.roles || [];
  const canAward = roles.some((r) =>
  ...
  const loadData = async () => {
    try {
      ...
      const promises: Promise<any>[] = [
        api.getAll<CoinTransaction>('coin-transaction', { expand: 'student.user,createdByUser' }),
        api.getAll<CoinRule>('coin-rule'),
      ];

      if (canAward) {
        promises.push(api.getAll<Student>('student', { expand: 'user' }));
      }

      const [txRes, ruleRes, sRes] = await Promise.all(promises);

      setTransactions(txRes?.items || []);
      setRules(ruleRes?.items || []);

      if (canAward && sRes?.items) {
  ...
  // Student bo'lsa faqat o'ziga tegishli tranzaksiyalarni filtrlash
  const filteredTransactions = isStudent
    ? transactions.filter((tx) => tx.student?.user_id === user?.id || (user as any)?.student_id === tx.student_id)
    : transactions;

  const totalCirculation = transactions.reduce((sum, tx) => sum + Number(tx.amount || 0), 0);
  const studentBalance = filteredTransactions.reduce((sum, tx) => sum + Number(tx.amount || 0), 0);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">
            {isStudent ? 'Mening Coinlarim (Rag\'bat Valyutasi)' : 'Maktab Valyutasi (Coin Ledger)'}
          </h1>
          <p className="text-slate-500 text-sm">
            {isStudent
              ? 'Akademik yutuqlaringiz, musobaqalar va darsdagi faolligingiz uchun to\'plangan balans'
              : 'O\'quvchilarning akademik yutuqlari, faolligi va namunali xulqini rag\'batlantiruvchi ichki valyuta'}
          </p>
        </div>
        {canAward && (
          <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">
              {isStudent ? 'Sizning Balansingiz' : 'Jami aylanmadagi Coin'}
            </p>
            <p className="text-2xl font-extrabold text-amber-600">
              {isStudent ? studentBalance : totalCirculation} coin
            </p>
          </div>
        </div>
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">
              {isStudent ? 'Tranzaksiyalaringiz' : 'Tranzaksiyalar Soni'}
            </p>
            <p className="text-2xl font-extrabold text-slate-900">{filteredTransactions.length} ta</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-200 flex items-center justify-between">
          <h2 className="text-base font-bold text-slate-800">
            {isStudent ? 'Mening Tranzaksiyalarim Tarixi' : 'Tranzaksiyalar Tarixi (Audit Jurnali)'}
          </h2>
          <span className="text-xs font-medium text-slate-500">{filteredTransactions.length} ta yozuv</span>
        </div>
        ...
        {loading ? (
          ...
          <div className="p-12 text-center text-slate-500">
            {isStudent ? 'Sizda hozircha hech qanday coin amallari yo\'q.' : 'Tranzaksiyalar mavjud emas.'}
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse text-sm">
              <thead>
                <tr className="bg-slate-50 text-slate-600 font-semibold border-b border-slate-200">
                  {!isStudent && <th className="py-3 px-4">O'quvchi</th>}
              ...
              <tbody className="divide-y divide-slate-200">
                {filteredTransactions.map((tx) => {
                  ...
                  const date = new Date(tx.created_at * 1000).toLocaleString('uz-UZ');
      ...
      {/* Award Modal - faqat canAward bo'lganlarga */}
      {awardModalOpen && canAward && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl max-w-md w-full p-6 shadow-xl space-y-4">
            <div className="flex items-center justify-between border-b pb-3">
              ...
              <button onClick={() => setAwardModalOpen(false)} className="text-slate-400 hover:text-slate-600">
                <X size={20} />
              </button>
            </div>
